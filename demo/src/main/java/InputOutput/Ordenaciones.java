...
public class Ordenaciones {
    ...
    public void ordenCronologico(ArrayList<Poblacion> poblaciones){
    ...
    public void intercambiarPoblaciones(ArrayList<Poblacion> poblaciones, int pos1, int pos2) {
        Poblacion aux = poblaciones.get(pos1);
        poblaciones.set(pos1, poblaciones.get(pos2));
        poblaciones.set(pos2, aux);
    }

    public void ordenNombrePoblacion(ArrayList<Poblacion> poblaciones){
        for(int i = 0; i< poblaciones.size()-1; i++) {
            for(int j = 0; j<poblaciones.size()-1-i; j++) {
